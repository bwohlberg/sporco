...
"""Usage example: cbpdndl.ConvBPDNMaskDcplDictLearn (colour images)."""
...
shp = np.pad(sh, ((0, Npr), (0, Npc), (0, 0), (0, 0)), mode='constant')
...
W[np.abs(W) > t] = 1;
W[np.abs(W) < t] = 0;
W = np.pad(W, ((0, Npr), (0, Npc), (0, 0), (0, 0)), mode='constant')
shpw = W * shp
...
lmbda = 0.05
L = 2e2
opt1 = cbpdndl.MixConvBPDNDictLearn.Options(
                     {'Verbose': True, 'MaxMainIter': 200,
                      'AccurateDFid': True,
                      'CBPDN': {'rho': 50.0*lmbda + 0.5},
                      'CCMOD': {'ZeroMean': True, 'BackTrack' : { 'Enabled' : True }, 'L' : L}})
d1 = cbpdndl.MixConvBPDNDictLearn(D0, shpw, lmbda, opt1)
D1 = d1.solve()
...
opt2 = cbpdndl.MixConvBPDNMaskDcplDictLearn.Options(
                     {'Verbose': True, 'MaxMainIter': 200,
                      'AccurateDFid': True,
                      'CBPDN': {'rho': 50.0*lmbda + 0.5},
                      'CCMOD': {'ZeroMean': True, 'BackTrack' : { 'Enabled' : True }, 'L' : L}})
Wr = np.reshape(W, W.shape[0:3] + (W.shape[3], 1))
d2 = cbpdndl.MixConvBPDNMaskDcplDictLearn(D0, shpw, lmbda, Wr, opt2)
D2 = d2.solve()
